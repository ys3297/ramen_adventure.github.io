...
Comments: According to the graph, it appears that most rated ramen varieties from China, South Korea, and USA have a peak of rating scores within 3.5 to 4 stars and a second but smaller peak at score 5. Among these four countries, Japan has a relatively higher proportion of rating scores within 4 to 5 stars.  
...
Comments: Among countries with 30 or more varieties rated, it appears that the top 3 average rating countries have average rating scores around 4.1 out of 5 stars. Malaysia had the highest average rating score (`r avg_num_country %>% filter(country == "Malaysia") %>% pull(avg)` out of 5) among all countries, followed by Singapore (`r avg_num_country %>% filter(country == "Singapore") %>% pull(avg)` out of 5) and Indonesia (`r avg_num_country %>% filter(country == "Indonesia") %>% pull(avg)` out of 5). These countries produced ramens with higher quality (better taste) despite that they were not the top ramen producers in varieties.  
...
We also noticed that Malaysia, Singapore, and Indonesia had the highest average ramen rating stars (all above 4 stars out of 5). This finding aligns with the result from TOP TEN analysis, where we found that Singapore had the most top 10 ramen varieties, followed by Malaysia and Japan. 

However, we are worried that the result from this dataset might be biased, considering that some countries in the dataset didn't have enough ramen varieties recorded and we had to exclude them from our analysis. Further analysis is needed on a dataset with larger sample size. 
